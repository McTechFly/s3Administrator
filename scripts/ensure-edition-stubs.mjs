...
function enforceCloudPricingPageRoute(projectRoot) {
  ...
  const content = [
    ...
    'export { metadata } from "@s3administrator/cloud/marketing/pages/pricing"',
    'export { default } from "@s3administrator/cloud/marketing/pages/pricing"',
    "",
  ].join("\n")
...
function cleanupCloudStubs(projectRoot) {
  // Remove cloud-generated stubs tracked in .s3admin-stubs
  const markerPath = resolve(projectRoot, ".s3admin-stubs")
  let removed = 0

  if (existsSync(markerPath)) {
    const stubs = readFileSync(markerPath, "utf-8")
      .split("\n")
      .map((l) => l.trim())
      .filter(Boolean)

    for (const stubPath of stubs) {
      const fullPath = resolve(projectRoot, stubPath)
  ...
  // Always remove cloud-specific files that may exist outside the marker
  const cloudOnlyFiles = [
    "src/app/(marketing)/pricing/page.tsx",
  ]

  for (const filePath of cloudOnlyFiles) {
    const fullPath = resolve(projectRoot, filePath)
    if (existsSync(fullPath)) {
      const content = readFileSync(fullPath, "utf-8")
      if (content.includes("@s3administrator/cloud")) {
        unlinkSync(fullPath)
        removed++
      }
    }
  ...
  if (removed > 0) {
    console.log(`✓ Cleaned up ${removed} cloud-generated stubs for community build`)
  }
}
...
export async function ensureEditionStubs(explicitEnvironment) {
  ...
  const cloudSetupPath = findFirstExisting([
    resolve(root, "..", "cloud-packages", "cloud", "bin", "s3admin-cloud-setup.mjs"),
    resolve(root, "node_modules/@s3administrator/cloud/bin/s3admin-cloud-setup.mjs"),
  ])

  if (environment === "CLOUD") {
    if (!cloudSetupPath) {
      throw new Error(
        "Cloud setup package not found. Install @s3administrator/cloud, add ../cloud-packages/cloud locally, or use ENVIRONMENT=COMMUNITY.",
      )
    }
